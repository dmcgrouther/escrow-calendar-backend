...
const login = async (req, res) => {
  try {
    ...
    if (isMatch) {
      ...
      return res.status(400).json({ status: 400, message: 'Email or password is correct' });
    }
  } catch {
    return res.status(500).json({ status: 500, error: 'Something has gone wrong with login. Please try again.' });
  }
};
...
const logout = async (req, res) => {
  try {
    req.session.destroy();
    return res.status(200).json({ status: 200, message: 'Succsful logout' });
  } catch {
    return res.status(400).json({ status: 400, error: 'Something has gone wrong, please try again' });
  }
}
